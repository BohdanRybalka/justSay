...
from app.stt.cloud import CloudSTTProvider
...
def test_factory_caches_provider():
    ...
    p1 = get_stt_provider(settings)
    p2 = get_stt_provider(settings)
    assert p1 is p2
...
def test_factory_invalidates_on_mode_change():
    ...
    p1 = get_stt_provider(cloud_settings)
    p2 = get_stt_provider(local_settings)
    assert p1 is not p2
    assert isinstance(p1, CloudSTTProvider)
...
def test_cloud_stt_model_name():
    ...
    provider = CloudSTTProvider(settings)
...
async def test_cloud_stt_transcribe(sample_wav):
    ...
    with patch.object(
        CloudSTTProvider, "_call_gemini", return_value=("  Привіт світ  ", None)
    ):
...
async def test_cloud_stt_tokens_used(sample_wav):
    ...
    with patch.object(CloudSTTProvider, "_call_gemini", return_value=("Привіт світ", 1500)):
        result = await provider.transcribe(sample_wav, language="uk")
...
async def test_cloud_stt_empty_response(sample_wav):
    ...
    with patch.object(CloudSTTProvider, "_call_gemini", return_value=(None, None)):
