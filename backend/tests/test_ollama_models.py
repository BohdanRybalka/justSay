...
from app.embeddings.ollama_models import list_models, model_matches


def test_model_matches_exact():
    assert model_matches("gemma3:4b", "gemma3:4b") is True


def test_model_matches_no_tag_target():
    assert model_matches("gemma3:4b", "gemma3") is True
    assert model_matches("gemma3:latest", "gemma3") is True


def test_model_matches_different_tag():
    assert model_matches("gemma3:latest", "gemma3:4b") is False


def test_model_matches_different_model():
    assert model_matches("llama3:8b", "gemma3:4b") is False
...
async def test_list_models_target_found():
    ...
    async with httpx.AsyncClient(
        ...
        models, found, size = await list_models(client, "gemma3:4b")
    assert len(models) == 2
